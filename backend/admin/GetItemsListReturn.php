	...
	<tr>
		...
		if($res = mysqli_query($conn,$sql_sel_items)){
			while ($row = mysqli_fetch_assoc($res)) {
				...
				echo "<td><input class='form-control' type='number' min='1' max = '{$row['item_amount']}' name='return-wornout-{$row['item_id']}' value='0'></td>";
				echo "</tr>";
			}
		}
		?>
...
<script type="text/javascript">
	$("#submit").click(function(event) {
		...
		var item_wornout = $("input[name^='return-wornout-']").serializeArray();

		$.post('item_return_save.php', {item_return: item_return,item_wornout: item_wornout, br_id:br_id}, function(data, textStatus, xhr) {
