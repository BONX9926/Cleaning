	...
	if(count($_POST) == 4) {
		...
		if ($_FILES['img']['size'] > 0) {
			if(move_uploaded_file($_FILES["img"]["tmp_name"],$temp_files.$_FILES["img"]["name"])) {
				$status = true;
				$sql = "UPDATE `item` SET `img`='{$_FILES['img']['name']}',`item_name`='{$_POST['item_name']}',`quantity_all`='{$_POST['quantity_all']}',`item_price`='{$_POST['item_price']}' WHERE `item_id`='{$_POST['item_id']}' ";
